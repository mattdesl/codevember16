...
function BloomPass (scene, camera, opt = {}) {
  ...
  this.postShader = new THREE.RawShaderMaterial({
    ...
    fragmentShader: glslify(path.resolve(__dirname + '/../shaders/bloom-blur.frag')),
    uniforms: {
  ...
  this.combineShader = new THREE.RawShaderMaterial({
    vertexShader: glslify(path.resolve(__dirname + '/../shaders/pass.vert')),
    fragmentShader: glslify(path.resolve(__dirname + '/../shaders/bloom-combine.frag')),
    uniforms: {
...
BloomPass.prototype = {
  ...
  render: function (renderer, writeBuffer, readBuffer, delta) {
    ...
    this.combineShader.uniforms.tBloomDiffuse.value = this._blurTarget.texture;
