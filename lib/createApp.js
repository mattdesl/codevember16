...
function createApp (opt = {}) {
  // Scale for retina
  const dpr = 4//Math.min(defined(opt.maxPixelRatio, 2), window.devicePixelRatio);
  ...
  function setupPost () {
    ...
    if (composer.initialRenderTarget.depthTexture) {
  ...
  function resize () {
    ...
    const clampSize = opt.clampSize !== false;
    if (clampSize) {
      const maxWidth = 720;
    ...
    app.left = Math.floor((window.innerWidth - width) / 2);
    app.top = Math.floor((window.innerHeight - height) / 2);
    canvas.style.left = `${app.left}px`;
    canvas.style.top = `${app.top}px`;
